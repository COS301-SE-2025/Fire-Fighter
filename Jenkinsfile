            ...
            steps {
                dir('FF-Angular') {
                    ...
                    sh '''
                        set -eo pipefail
                        export CI=true
                        echo "🔎 Ensuring Chrome is available for headless tests..."
                        if ! command -v google-chrome >/dev/null 2>&1; then
                            echo "📥 Installing Chrome via Puppeteer (no root required)..."
                            npx --yes puppeteer browsers install chrome >/dev/null 2>&1 || true
                            export CHROME_BIN=$(npx --yes puppeteer browsers path chrome)
                        else
